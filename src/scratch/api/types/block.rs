...
impl<'de> Visitor<'de> for BlockVisitor {
		...
		where A: SeqAccess<'de>,
    {
		...
		return match elements
			...
			"doPlaySoundAndWait" => {
				...
				std::mem::swap(&mut music, elements.get_mut(1).ok_or(serde::de::Error::invalid_length(1, &self))?);
				Ok(Block::PlaySoundAndWait(PlaySoundAndWait{
					song: music
				}))
			},
			_=> {
	...
	{
		let key: &str = map.next_key()?.ok_or(serde::de::Error::invalid_length(0, &self))?;
		match key{
			"PlaySoundAndWait" => {
