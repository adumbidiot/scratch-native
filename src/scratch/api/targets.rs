...
use self::super::types::{Project, Block};
use self::super::types::json::{ProjectJson, ScriptJson};
...
impl JsTarget {
	...
	pub fn get_sprite_js(sprite: &super::types::json::SpriteJson) -> String{
		let costumes = sprite
			.costumes
			.iter()
			.fold(String::new(), |code, costume| code + &Self::get_costume_js(sprite, costume));
...
//End {name}
"#, name = sprite.name, costumes=costumes, x = sprite.x, y = sprite.y, scripts = scripts_str);
	}
...
"#, 
	...
	pub fn get_main_js(project: &ProjectJson) -> String{
		...
		sound_list.extend(project.sounds.iter()
			.chain(project.children.iter()
				.flat_map(|child| child.sounds.iter())));
		
		let mut audio_data = String::new();
		for audio in sound_list.iter(){
			audio_data += &format!("game.audioAssets.set('{name}', new Audio('assets/audio/{md5}'));\n", name=audio.name, md5=audio.src);
		}
...
{body}
game.start();
"#, body=body, audio_data=audio_data);
		return main_js;
	}
	...
	pub fn get_scratch_block_js(block: &Block) -> String{
		...
		match block {
			Block::PlaySoundAndWait(ref data) => {
				total += &format!(r#"if(game.data.get("GAME_FIRST_CYCLE")){{
