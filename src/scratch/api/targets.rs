...
use fs_extra::dir::{copy, CopyOptions, create};
...
impl Target for JsTarget {
	...
	fn init(&mut self, project: &mut Project, path: &PathBuf) -> TargetResult<()>{
		...
		runtime_path.pop();
		runtime_path.pop();
		runtime_path.push("lib");
		runtime_path.push("scratch-js");
		runtime_path.push("scratch-js.js");
		...
		for sprite in &project.project_json.children{
...
"#, body);
		...
		target_dir.push("main.js");
		Self::write_file(&target_dir, &main_js.into_bytes()).expect("Could not save file");
		target_dir.pop();
  ...
  "scripts": {{
	"test": "node server.js"
		...
		target_dir.push("package.json");
		Self::write_file(&target_dir, &package_json.into_bytes()).expect("Could not save file");
		target_dir.pop();
...
"#);
		target_dir.push("server.js");
		Self::write_file(&target_dir, &server_js.into_bytes()).expect("Could not save file");
		target_dir.pop();
...
"#, title = &name);
		target_dir.push("index.html");
		Self::write_file(&target_dir, &index_html.into_bytes()).expect("Could not save file");
		target_dir.pop();
